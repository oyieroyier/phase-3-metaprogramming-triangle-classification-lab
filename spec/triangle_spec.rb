describe Triangle do
  describe "#kind" do
    ...
    it "raises an error for triangles with no size" do
      expect { Triangle.new(0, 0, 0).kind }.to raise_error(
        Triangle::TriangleError
      )
    end

    it "raises an error for triangles with negative sides" do
      expect { Triangle.new(3, 4, -5).kind }.to raise_error(
        Triangle::TriangleError
      )
    end

    it "raises an error for triangles violating triangle inequality" do
      expect { Triangle.new(1, 1, 3).kind }.to raise_error(
        Triangle::TriangleError
      )
      expect { Triangle.new(2, 4, 2).kind }.to raise_error(
        Triangle::TriangleError
      )
      expect { Triangle.new(7, 3, 2).kind }.to raise_error(
        Triangle::TriangleError
      )
    end
  end
